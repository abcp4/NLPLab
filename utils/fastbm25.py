...
class fastbm25(object):
    ...
    def top_k_sentence_weighted(self,document,weights,k=1):
      """
      document: Iterable, to be retrieved
      Returns
        -------
      ...
      for word in document:
          ...
          for key, value in self.document_score[word].items():
              if key not in score_overall:
                  # print(score_overall)
                  score_overall[key] = value*weights[c]
              else:
                  score_overall[key] += value*weights[c]
    ...
    def top_k_indexes(self,document,k=1):
      """
      document: Iterable, to be retrieved
      Returns
        -------
      ...
      for word in document:
          ...
          for key, value in self.document_score[word].items():
              if key not in score_overall:
                  ...
                  score_overall[key] += value
      k_keys_sorted = heapq.nlargest(k, score_overall,key=score_overall.__getitem__)
      return [(item,item,score_overall.get(item,None)) for item in k_keys_sorted]
